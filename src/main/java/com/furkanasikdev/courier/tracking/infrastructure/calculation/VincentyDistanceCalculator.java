...
/**
 * Vincenty formula ile iki koordinat arası mesafe hesaplama.
 *
 * @see <a href="https://www.baeldung.com/java-find-distance-between-points">Baeldung - Distance Between Points</a>
 */
@Component("vincenty")
@ConditionalOnProperty(name = "app.calculation.distance-strategy", havingValue = "vincenty")
public class VincentyDistanceCalculator implements DistanceCalculationStrategy {
	...
	public double calculateDistance(Location from, Location to) {
		...
		double U1 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(from.lat())));
		double U2 = Math.atan((1 - FLATTENING) * Math.tan(Math.toRadians(to.lat())));
		double sinU1 = Math.sin(U1);
		double cosU1 = Math.cos(U1);
		double sinU2 = Math.sin(U2);
		double cosU2 = Math.cos(U2);

		double longitudeDifference = Math.toRadians(to.lng() - from.lng());
		...
		do {
			...
			longitudeDifference = Math.toRadians(to.lng() - from.lng()) + (1 - C) * FLATTENING * sinAlpha *
